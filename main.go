...
import (
	...
	"code.internetisalie.net/gonew/internal/edit"
...
func main() {
	...
	// check the workspace for the module
	var stdout, stderr bytes.Buffer
	cmd := exec.Command("go", "list", "-json", "-m")
	cmd.Stdout = &stdout
	cmd.Stderr = &stderr
	if err = cmd.Run(); err != nil {
		log.Fatalf("go list -json -m: %v\n", err)
	}
	...
	for decoder.More() {
		...
		if err = decoder.Decode(&check); err != nil {
			log.Fatalf("go list -json -m: invalid JSON output: %v\n%s%s", err, stderr.Bytes(), stdout.Bytes())
		}
		if check.Path == srcMod {
			info = check
			break
		}
	}
	...
	filepath.WalkDir(info.Dir, func(src string, d fs.DirEntry, err error) error {
		...
		if d.IsDir() {
			if rel == ".git" {
				// since we support workspace folders
